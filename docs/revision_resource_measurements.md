...
| Public end-to-end true-2-D demonstration | 10.284674577 s | 752,074,752 bytes (717.234 MiB) | 403,456 bytes across 23 output files |
...
The demonstration ran in a clean Ubuntu WSL2 Python 3.12.3 environment on the
same host, with PyTorch 2.13.0+cpu, NumPy 2.2.6, and one computation thread.
The WSL environment exposed 33,308,188,672 bytes of memory. Both measurements
...
were 3,504 for P4 and 203 for the demonstration. An independent audit of the
...
The demonstration's **10.284674577 s** likewise covers the complete command:
...
`pre_postflight_wall_seconds=7.509622985` does not include the complete workflow
and should not replace the profiled command duration. Individual internal
stage times are preserved in the copied `demo_run_summary.json`.
...
For the demonstration, all output files total **403,456 bytes**, including
...
**13,391 bytes**. Its small output and runtime do not describe the cost of
...
original evidence paths. The folder also contains copies of both profiles,
their raw memory CSVs and stdout, the original/new P4 manifests, P4 generation
summary, and demonstration provenance/stage summary. `checksums.json` binds
...
audit. All costs above come from the two completed profiled executions.
